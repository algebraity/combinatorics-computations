...
static inline int ilog2_int(int x) {
    if (x <= 0) return 0;
...
void A_ads_size_big(int n, mpz_t A_size, mpz_t AA_size) {
    mpz_set_ui(A_size, 0);
    mpz_set_ui(AA_size, 0);

    int num_odds = (n + 1) / 2;
    int *odds = (int *)malloc((size_t)num_odds * sizeof(int));
    int *Emax = (int *)malloc((size_t)num_odds * sizeof(int));
    if (!odds || !Emax) { perror("malloc"); exit(1); }
    ...
    // Map u -> mask
    UMap map;
    umap_init(&map, 1u << 12); // 4096 start; grows as needed

    // Temporaries
    mpz_t aZ, bZ, K, u, tmp_pow, tmp_interval;
    mpz_init(aZ);
    mpz_init(bZ);
    mpz_init(K);
    mpz_init(u);
    ...
    for (int ai = 0; ai < num_odds; ai++) {
        int a = odds[ai];
        int Ea = Emax[ai];
        mpz_set_ui(aZ, (unsigned long)a);

        for (int bi = 0; bi < num_odds; bi++) {
            int b = odds[bi];
            int Eb = Emax[bi];
            ...
            for (int d = 0; d <= max_d; d++) {
                ...
                // K = a + (b << d) as big-int
                mpz_mul_2exp(K, bZ, (mp_bitcnt_t)d);
                mpz_add(K, K, aZ);

                // tz = v2(K) = number of trailing zeros in K
                mp_bitcnt_t tz = mpz_scan1(K, 0); // index of least significant 1-bit

                // u = K >> tz
                mpz_tdiv_q_2exp(u, K, tz);

                unsigned t_lo = (unsigned)(1 + tz);
                unsigned t_hi = (unsigned)(E1max + (int)tz);

                mpz_t *maskp = umap_get_or_create(&map, u);
...
int main(int argc, char *argv[]) {
    ...
    A_ads_size_big(n, A_size, AA_size);
